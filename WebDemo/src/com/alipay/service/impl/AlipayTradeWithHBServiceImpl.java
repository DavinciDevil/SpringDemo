...
import com.alipay.service.impl.hb.HbListener;
import com.alipay.service.impl.hb.TradeListener;
 ...
 *
 *  一定要在创建AlipayTradeService之前调用CommonValue.init("alipayrisk10");设置参数
 *
 */
    ...
    public static class ClientBuilder {
        ...
        public AlipayTradeWithHBServiceImpl build() {
            if (StringUtils.isEmpty(gatewayUrl)) {
                gatewayUrl = CommonValue.getOpenApiDomain(); // 与mcloudmonitor网关地址不同
            }
            if (StringUtils.isEmpty(appid)) {
                appid = CommonValue.getAppid();
            }
            if (StringUtils.isEmpty(privateKey)) {
                privateKey = CommonValue.getPrivateKey();
            }
            ...
            if (StringUtils.isEmpty(alipayPublicKey)) {
                alipayPublicKey = CommonValue.getAlipayPublicKey();
            }
            if (StringUtils.isEmpty(signType)) {
                signType = CommonValue.getSignType();
            }
            if (listener == null) {
...
//                builder.getFormat(), builder.getCharset(), builder.getAlipayPublicKey(), builder.getSignType());
    ...
    public AlipayF2FPayResult tradePay(AlipayTradePayRequestBuilder builder) {
        ...
        System.out.println("trade.pay bizContent:" + request.getBizContent());
